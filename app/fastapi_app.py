...
app = FastAPI(title = "Godfather Word2vec API", lifespan = lifespan)    # This is the framework for the uvicorn server.
...
def get_similarity(w1: str, w2: str):
    ...
    if w1 not in model_wv or w2 not in model_wv:
        raise HTTPException(status_code=404, detail = "One of the words is missing from the vocabulary.")
